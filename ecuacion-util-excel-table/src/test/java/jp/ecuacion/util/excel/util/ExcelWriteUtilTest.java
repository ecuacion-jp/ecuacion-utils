...
public class ExcelWriteUtilTest {
  ...
  public void getReadyToEvaluateFormulaTest()
    ...
    cell = sheet.getRow(2).getCell(2);
    ExcelWriteUtil.getReadyToEvaluateFormula(cell, true, false, true, new String[0]);
    // changed
    Assertions.assertEquals(CellType.NUMERIC, cell.getCellType());
    Assertions.assertEquals(1, cell.getNumericCellValue());
  ...
  public void evaluateFormulaTest() throws EncryptedDocumentException, IOException {
    ...
    try {
      ...
      Assertions.assertEquals(
          ...
          ex.getMessageId());
      Assertions.assertEquals("B4",
          Objects.requireNonNull(ex.getCell()).getAddress().formatAsString());
    }
    ...
    try {
      ...
      Assertions.assertEquals("jp.ecuacion.util.excel.ExcelWriteUtil.DetailUnknown.message",
          ex.getMessageId());
      Assertions.assertEquals("B5",
          Objects.requireNonNull(ex.getCell()).getAddress().formatAsString());
    }
    ...
    try {
      ...
      Throwable rootCause = Objects.requireNonNull(
          Objects.requireNonNull(ex.getCause()).getCause()).getCause();
      Assertions.assertEquals(true, rootCause instanceof WorkbookNotFoundException);
      Assertions.assertEquals(
          "jp.ecuacion.util.excel.ExcelWriteUtil.WorkbookNotFoundException.message",
          ex.getMessageId());
      Assertions.assertEquals("B6",
          Objects.requireNonNull(ex.getCell()).getAddress().formatAsString());
    }
