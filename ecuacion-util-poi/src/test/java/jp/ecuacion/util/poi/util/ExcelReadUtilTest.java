...
public class ExcelReadUtilTest {
  ...
  public void readFromCellTest()
    ...
    assertEquals("12:34:56",
        reader.dateTimeFormat("HH:mm:ss").getStringFromCell(sheet.getRow(row++).getCell(dataCol)));
    assertEquals("2000/1/23 12:34:56", reader.dateTimeFormat("yyyy/M/dd HH:mm:ss")
        .getStringFromCell(sheet.getRow(row++).getCell(dataCol)));
    assertEquals("あいう", reader.dateTimeFormat("yyyy-MM-dd")
        .getStringFromCell(sheet.getRow(row++).getCell(dataCol)));
